...
function isRealDate(value: string) { const match = /^(\d{4})-(\d{2})-(\d{2})$/.exec(value); if (!match) return false; const parsed = new Date(`${value}T00:00:00`); return !Number.isNaN(parsed.getTime()) && parsed.getFullYear() === Number(match[1]) && parsed.getMonth() + 1 === Number(match[2]) && parsed.getDate() === Number(match[3]); }
  ...
  return <main className="module-shell">
    ...
    {error && <FeedbackMessage onRetry={() => void load()}>{error}</FeedbackMessage>}
    <section className="module-toolbar"><div className="module-search"><Search size={17} /><input aria-label="Buscar funcionário, matrícula ou material" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar funcionário, matrícula ou material" /></div><select aria-label="Filtrar solicitações por tipo" value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)}><option value="all">Todos os tipos</option>{Object.entries(requestLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select><select aria-label="Filtrar solicitações por status" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="all">Todos os status</option>{Object.entries(statusLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></section>
    {!loading && <section className="module-summary"><div><strong>{requests.length}</strong><span>solicitações</span></div><div><strong>{requests.filter((item) => item.status === "pending").length}</strong><span>pendentes</span></div><div><strong>{requests.filter((item) => item.status === "in_review").length}</strong><span>em análise</span></div><div><strong>{requests.filter((item) => item.status === "completed").length}</strong><span>concluídas</span></div></section>}
