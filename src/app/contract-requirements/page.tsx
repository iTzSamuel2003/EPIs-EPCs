...
type Requirement = { id: string; scenario_id: string; source_annex: string; quantity: number; unit: string; usage_scope: "individual" | "coletivo"; notes: string | null; materials: { id: string; name: string; type: "EPI" | "EPC" | "FERRAMENTAL"; contract_item_code: string | null; contract_category: string; contract_specification: string | null } | null };
...
export default function ContractRequirementsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: scenarioData, error: scenarioError }, { data: requirementData, error: requirementError }, { data: lotData, error: lotError }] = await Promise.all([
        ...
        supabase.from("contract_requirements").select("id, scenario_id, source_annex, quantity, unit, usage_scope, notes, materials(id, name, type, contract_item_code, contract_category, contract_specification)"),
  ...
  return <main className="module-shell">
    ...
    {loading ? <div className="module-loading"><LoaderCircle className="spin" size={22} /> Carregando requisitos...</div> : <>
      ...
      <section className="module-summary"><div><ClipboardCheck size={18} /><strong>{visible.length}</strong><span>itens exigidos</span></div><div><PackageCheck size={18} /><strong>{totalRequired}</strong><span>quantidade mínima</span></div><div><ShieldAlert size={18} /><strong>{deficitCount}</strong><span>com saldo insuficiente</span></div><div><FileText size={18} /><strong>{selected?.source_annex ?? "—"}</strong><span>fonte contratual</span></div></section>
      <section className="panel module-table-card"><div className="panel-header"><div><h2>Materiais mínimos do cenário</h2><p>Os itens coletivos atendem à equipe; os individuais devem ser conferidos por colaborador.</p></div></div>{visible.length ? <div className="table-wrap"><table><thead><tr><th>ITEM CONTRATUAL</th><th>CATEGORIA</th><th>USO</th><th>MÍNIMO</th><th>DISPONÍVEL</th><th>SITUAÇÃO</th></tr></thead><tbody>{visible.map((item) => { const available = stock[item.materials?.id ?? ""] ?? 0; const enough = available >= item.quantity; return <tr key={item.id}><td><div className="material-cell"><div className="material-type-icon epi"><ClipboardCheck size={15} /></div><div><strong>{item.materials?.name ?? "Material não encontrado"}</strong><small>{item.materials?.contract_item_code ?? "Sem código"}</small></div></div></td><td><span className="type-badge epi">{categoryLabel[item.materials?.contract_category ?? ""] ?? item.materials?.type ?? "—"}</span></td><td>{item.usage_scope === "individual" ? "Individual" : "Coletivo"}</td><td><strong>{item.quantity} {item.unit}</strong></td><td><strong className={enough ? "history-positive" : "history-negative"}>{available} {item.unit}</strong></td><td><span className={`status-pill ${enough ? "success" : "danger"}`}>{enough ? "Atende" : "Repor estoque"}</span></td></tr>})}</tbody></table></div> : <div className="empty-state"><ClipboardCheck size={27} /><strong>Nenhum requisito cadastrado</strong><span>Selecione outro cenário ou importe os itens do anexo contratual.</span></div>}</section>
    </>}
  </main>;
}
