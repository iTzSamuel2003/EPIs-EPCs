...
export default function ReturnsPage() {
  ...
  function updateLine(id: string, field: keyof ReturnLine, value: string | number | boolean) { setLines((current) => ({ ...current, [id]: { ...current[id], [field]: value, ...(field === "incident_type" ? { deduction_requested: value !== "normal" } : {}) } })); }
  ...
  async function submit(event: FormEvent) {
    ...
    if (selectedLines.some((line) => line.incident_type !== "normal" && !signature.trim())) {
      setError("Informe o nome completo do responsável para registrar uma ocorrência.");
      return;
    }
