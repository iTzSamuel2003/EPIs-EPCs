...
type Actor = { id: string; full_name: string | null };
...
export default function AuditPage() {
  ...
  async function load() {
    ...
    try {
      ...
      if (actorIds.length) {
        const { data: actorData, error: actorError } = await supabase.from("profiles").select("id,full_name").in("id", actorIds);
        if (!mountedRef.current || requestId !== loadRequestRef.current) return;
        if (actorError) {
          setActors({});
  ...
  const filtered = useMemo(() => rows.filter((row) => {
    ...
    const text = `${actionLabels[row.action] ?? row.action} ${tableLabels[row.table_name] ?? row.table_name} ${row.record_id ?? ""} ${actor?.full_name ?? ""}`.toLowerCase();
