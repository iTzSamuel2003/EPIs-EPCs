...
type EntryItem = { material_id: string; materialQuery: string; lot_number: string; quantity: string; unit_cost: string; manufactured_at: string; expires_at: string };
const newItem = (): EntryItem => ({ material_id: "", materialQuery: "", lot_number: "", quantity: "1", unit_cost: "0", manufactured_at: "", expires_at: "" });
...
export default function EntriesPage() {
  ...
  async function loadMaterials() {
    const { data, error: loadError } = await createClient().from("materials").select("id, name, internal_code, unit").eq("status", "active").order("name");
    if (loadError) setError(loadError.message);
    else setMaterials((data ?? []) as MaterialOption[]);
    setLoading(false);
  }
  ...
  function selectMaterial(index: number, material: MaterialOption) {
    setItems((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, material_id: material.id, materialQuery: material.name } : item));
  ...
  function chooseFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (file && !["application/pdf", "image/jpeg", "image/png", "image/webp"].includes(file.type)) {
      setError("Anexe a nota em PDF, JPG, PNG ou WEBP."); setInvoiceFile(null); return;
    }
    if (file && file.size > 10 * 1024 * 1024) {
      setError("A nota fiscal deve ter no máximo 10 MB."); setInvoiceFile(null); return;
    }
    setInvoiceFile(file);
  }
  ...
  return <main className="module-shell">
    ...
    <section className="panel entry-card"><div className="entry-intro"><div className="entry-icon"><ClipboardList size={22} /></div><div><h2>Nota fiscal e itens recebidos</h2><p>Informe a nota uma vez e adicione todos os produtos e lotes relacionados.</p></div></div>
      <form className="material-form" onSubmit={submit}>
        ...
        <div className="delivery-items-header"><div><h3>Produtos da nota</h3><p>Uma linha para cada material ou lote recebido.</p></div><button type="button" className="secondary-button" onClick={() => setItems((current) => [...current, newItem()])}><Plus size={15} /> Adicionar produto</button></div>
        <div className="delivery-items">{items.map((item, index) => <div className="delivery-item-row entry-item-row" key={index}><span className="item-index">{index + 1}</span>
