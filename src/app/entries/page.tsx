...
export default function EntriesPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    if (invoiceFile && !invoiceNumber.trim()) { setError("Informe o nÃºmero da nota fiscal antes de anexar o arquivo."); return; }
  ...
  return <main className="module-shell"><header className="module-header"><div><p className="eyebrow">MOVIMENTAÃ‡ÃƒO DE ESTOQUE</p><h1>Entrada de materiais</h1><p className="module-subtitle">Registre vÃ¡rios produtos da mesma nota fiscal em uma Ãºnica operaÃ§Ã£o.</p></div><Link className="secondary-button" href="/stock"><ArrowLeft size={16} /> Ver estoque</Link></header>{success && <div className="feedback success-feedback"><Check size={17} /> {success}</div>}{error && <div className="feedback error-feedback"><X size={17} /> {error}</div>}<section className="panel entry-card"><div className="entry-intro"><div className="entry-icon"><ClipboardList size={22} /></div><div><h2>Nota fiscal e itens recebidos</h2><p>Informe a nota uma vez e adicione todos os produtos e lotes relacionados.</p></div></div><form className="material-form" onSubmit={submit}><div className="form-grid two"><label>NÃºmero da nota fiscal<input value={invoiceNumber} onChange={(event) => setInvoiceNumber(event.target.value)} placeholder="NF-000123" /></label><label>Data da entrada<input type="date" value={entryDate} onChange={(event) => setEntryDate(event.target.value)} required /></label></div><label className="file-upload">Foto ou PDF da nota fiscal<input type="file" accept="image/jpeg,image/png,image/webp,application/pdf" onChange={chooseFile} /><span><FileImage size={18} /> {invoiceFile ? invoiceFile.name : "Selecionar arquivo (mÃ¡x. 10 MB)"}</span></label><div className="delivery-items-header"><div><h3>Produtos da nota</h3><p>Uma linha para cada material ou lote recebido.</p></div><button type="button" className="secondary-button" onClick={() => setItems((current) => [...current, newItem()])}><Plus size={15} /> Adicionar produto</button></div><div className="delivery-items">{items.map((item, index) => <div className="delivery-item-row" key={index}><span className="item-index">{index + 1}</span><label>Material<select value={item.material_id} onChange={(event) => updateItem(index, "material_id", event.target.value)} required disabled={loading}><option value="">Selecione</option>{materials.map((material) => <option key={material.id} value={material.id}>{material.internal_code} Â· {material.name} ({material.unit})</option>)}</select></label><label>Lote<input value={item.lot_number} onChange={(event) => updateItem(index, "lot_number", event.target.value)} placeholder="LOTE-2026-001" required /></label><label>Quantidade<input type="number" min="1" value={item.quantity} onChange={(event) => updateItem(index, "quantity", event.target.value)} required /></label><label>Validade<input type="date" value={item.expires_at} onChange={(event) => updateItem(index, "expires_at", event.target.value)} /></label>{items.length > 1 && <button type="button" className="remove-item" aria-label={`Remover produto ${index + 1}`} onClick={() => setItems((current) => current.filter((_, itemIndex) => itemIndex !== index))}><Trash2 size={17} /></button>}</div>)}</div><label>ObservaÃ§Ãµes da nota<textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Fornecedor, conferÃªncia ou outras observaÃ§Ãµes" rows={3} /></label><div className="modal-actions"><button type="reset" className="secondary-button" onClick={() => { setItems([newItem()]); setInvoiceNumber(""); setInvoiceFile(null); setNotes(""); setError(""); }}>Limpar</button><button className="primary-button" disabled={saving || loading || !materials.length}>{saving ? "Registrando..." : "Registrar entrada"}<Plus size={16} /></button></div></form></section></main>;
}
