...
export default function EntriesPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    if (entryError) { if (uploadedPath) await supabase.storage.from("invoice-attachments").remove([uploadedPath]); setError(entryError.message); } else { setSuccess(items.length > 1 ? "Nota fiscal e itens registrados com sucesso." : "Entrada registrada e nota fiscal anexada com sucesso."); setEntriesRefreshKey((value) => value + 1); setItems([newItem()]); setInvoiceNumber(""); setEntryDate(new Date().toISOString().slice(0, 10)); setInvoiceFile(null); setNotes(""); }
    setSaving(false);
  }
