...
type MaterialForm = {
  ...
  useful_life_months: string; minimum_stock: string; location: string; notes: string; status: "active" | "inactive";
};

const emptyForm: MaterialForm = {
  internal_code: "", name: "", type: "EPI", unit: "un.", description: "", brand: "", manufacturer: "",
  model: "", ca_number: "", ca_expires_at: "", useful_life_months: "", minimum_stock: "0",
...
export default function MaterialsPage() {
  ...
  async function loadMaterials() {
    ...
    const [{ data, error: materialsError }, { data: lots, error: lotsError }] = await Promise.all([
      supabase.from("materials").select("id,internal_code,name,type,unit,description,brand,manufacturer,model,ca_number,ca_expires_at,useful_life_months,minimum_stock,location,notes,status").order("name"),
