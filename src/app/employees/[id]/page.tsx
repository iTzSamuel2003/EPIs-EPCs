...
type Item = { id: string; quantity: number; returned: number; expected_replacement_at: string | null; material: { name: string; internal_code: string | null; unit: string } | null; lot: { lot_number: string } | null; units: MaterialUnit[]; delivery: { id: string; delivered_at: string; reason: string; term_file_path: string | null; term_uploaded_at: string | null; term_signature_method: string | null } | null };
...
function EmployeeTermActions({ children }: { children: ReactNode }) {
  const [open, setOpen] = useState(false);
  const menuRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    if (!open) return;
    const closeWhenOutside = (event: MouseEvent) => { if (menuRef.current && !menuRef.current.contains(event.target as Node)) setOpen(false); };
    document.addEventListener("mousedown", closeWhenOutside);
    return () => document.removeEventListener("mousedown", closeWhenOutside);
  }, [open]);
  return <div className="employee-term-menu" ref={menuRef}><button type="button" className="action-button employee-term-menu-trigger" onClick={() => setOpen((current) => !current)} aria-expanded={open}>Ações do termo</button>{open && <div className="employee-term-menu-panel">{children}</div>}</div>;
}
...
export default function EmployeeMaterialsPage() {
  ...
  const [returnSheets, setReturnSheets] = useState<ReturnSheet[]>([]);
  ...
  useEffect(() => {
    async function load() {
      ...
      if (employeeError || deliveryError || courseError || returnError) setError(employeeError?.message ?? deliveryError?.message ?? courseError?.message ?? returnError?.message ?? "Não foi possível carregar a ficha.");
      setEmployee(employeeData as Employee);
      setCourses((courseData ?? []) as Course[]);
      setReturnSheets(((returnData ?? []) as Array<Record<string, unknown>>).map((row) => ({ id: String(row.id), returned_at: String(row.returned_at), reason: String(row.reason), term_file_path: row.term_file_path as string | null, term_signature_method: row.term_signature_method as string | null, term_signed_at: row.term_signed_at as string | null, items: ((row.return_items ?? []) as Array<Record<string, unknown>>).map((item) => ({ quantity: Number(item.quantity), material: item.material as ReturnSheet["items"][number]["material"], variant: ((item.delivery_item as Record<string, unknown> | null)?.variant ?? null) as ReturnSheet["items"][number]["variant"] })) })));
      setItems(((deliveryData ?? []) as unknown[]).map((row) => {
        ...
        return { id: String(value.id), quantity: Number(value.quantity), returned: ((value.return_items ?? []) as Array<Record<string, unknown>>).reduce((total, item) => total + Number(item.quantity || 0), 0), expected_replacement_at: value.expected_replacement_at as string | null, material, lot: value.material_lots as Item["lot"], units, delivery: value.deliveries as Item["delivery"] };
      }));
      setLoading(false);
    }
    void load();
  ...
  const currentItems = useMemo(() => items.filter((item) => item.quantity > item.returned), [items]);
  ...
  async function uploadReturnTerm(sheet: ReturnSheet, event: ChangeEvent<HTMLInputElement>) {
    ...
    setUploadingId(sheet.id); setError(""); const supabase = createClient(); const { data: profileData, error: profileError } = await supabase.from("profiles").select("organization_id").single();
    if (profileError || !profileData?.organization_id) { setError(profileError?.message ?? "Não foi possível identificar a organização."); setUploadingId(""); return; }
    const path = `${profileData.organization_id}/${sheet.id}/${crypto.randomUUID()}-${safeFileName(file.name)}`;
    const { error: uploadError } = await supabase.storage.from("delivery-terms").upload(path, file, { contentType: file.type, upsert: false });
    if (uploadError) { setError(uploadError.message); setUploadingId(""); return; }
    const { data: auth, error: authError } = await supabase.auth.getUser(); if (authError || !auth.user) { await supabase.storage.from("delivery-terms").remove([path]); setError(authError?.message ?? "Sua sessão expirou. Entre novamente."); setUploadingId(""); return; }
  ...
  async function downloadReturnTerm(sheet: ReturnSheet) {
    ...
    doc.setFontSize(9); doc.text(`Colaborador: ${employee!.full_name}`, margin, y); doc.text(`Matrícula: ${employee!.registration || "Não informada"}`, margin + 95, y); y += 6; doc.text(`Data: ${date(sheet.returned_at)} · Motivo: ${reasonLabels[sheet.reason] || sheet.reason}`, margin, y); y += 12;
    doc.setFont("helvetica", "bold"); doc.text("Materiais devolvidos", margin, y); y += 8; doc.setFont("helvetica", "normal"); sheet.items.forEach((item) => { const variation = item.variant ? ` · Tamanho ${item.variant.size || item.variant.name}` : ""; doc.text(`${item.quantity}x ${item.material?.name || "Material"}${variation}`, margin, y); y += 6; });
    y += 12; doc.text("Declaro que os materiais acima foram devolvidos e conferidos.", margin, y); y += 35; doc.line(margin, y, margin + 72, y); doc.line(120, y, 192, y); doc.setFontSize(8); doc.text("Assinatura do colaborador", margin + 36, y + 5, { align: "center" }); doc.text("Responsável pelo controle", 156, y + 5, { align: "center" }); doc.save(`termo-devolucao-${safeFileName(employee!.full_name)}.pdf`);
  }

  async function openSignedReturnTerm(sheet: ReturnSheet) {
    if (!sheet.term_file_path) return; const { data, error: signedUrlError } = await createClient().storage.from("delivery-terms").createSignedUrl(sheet.term_file_path, 300); if (signedUrlError || !data?.signedUrl) { setError(signedUrlError?.message ?? "Não foi possível abrir o termo assinado."); return; } window.open(data.signedUrl, "_blank", "noopener,noreferrer");
  }
  ...
  return <main className="module-shell employee-sheet">
    ...
    <section className="panel sheet-card enhanced-sheet">
      ...
      <section className="sheet-employee-heading"><div className="sheet-person-icon"><UserRound size={23} /></div><div><p>COLABORADOR</p><h2>{employee.full_name}</h2><span>{employee.function_name || employee.job_title || "Função não informada"}{employee.function_classification ? ` · Classe ${employee.function_classification}` : ""} · {employee.department || "Setor não informado"}</span></div><div className="sheet-unit"><span>UNIDADE</span><strong>{employee.unit || "Não informada"}</strong></div></section>
      <div className="employee-summary sheet-details"><div><span>Matrícula</span><strong>{employee.registration || "—"}</strong></div><div><span>CPF</span><strong>{employee.cpf || "—"}</strong></div><div><span>Setor</span><strong>{employee.department || "—"}</strong></div><div><span>Função</span><strong>{employee.function_name || employee.job_title || "—"}{employee.function_classification ? ` · Classe ${employee.function_classification}` : ""}</strong></div></div>
      ...
      <section className="sheet-section employee-delivery-sheets"><div className="sheet-section-heading"><div><p>FICHAS DE ENTREGA</p><h2>Termos de entrega</h2><span>{deliverySheets.length} ficha(s) registrada(s)</span></div></div>{deliverySheets.length ? <div className="table-wrap"><table><thead><tr><th>DATA</th><th>MOTIVO</th><th>ITENS</th><th>DOCUMENTO</th><th>AÇÕES</th></tr></thead><tbody>{deliverySheets.map((sheet) => <tr key={sheet.id}><td>{date(sheet.delivered_at)}</td><td>{reasonLabels[sheet.reason] || sheet.reason}</td><td>{sheet.items}</td><td><span className={`status-pill ${sheet.term_file_path ? "success" : "warning"}`}>{sheet.term_file_path ? <><Check size={11} /> {sheet.term_signature_method === "assisted" ? "Assinado" : "Anexado"}</> : "Pendente"}</span></td><td><EmployeeTermActions><div className="employee-sheet-actions"><button type="button" className="action-button" onClick={() => downloadTerm(sheet)}><Download size={14} /> Baixar termo para assinatura</button>{!sheet.term_file_path && <DeliverySignatureModal deliveryId={sheet.id} employeeName={employee.full_name} employeeCpf={employee.cpf} employeeRegistration={employee.registration} deliveredAt={sheet.delivered_at} reason={sheet.reason} items={sheet.materials.map((item) => ({ quantity: item.quantity, expected_replacement_at: item.expected_replacement_at, material: item.material }))} currentPath={sheet.term_file_path} onComplete={(path) => setItems((current) => current.map((item) => item.delivery?.id === sheet.id ? { ...item, delivery: item.delivery ? { ...item.delivery, term_file_path: path, term_signature_method: "assisted" } : item.delivery } : item))} />}</div></EmployeeTermActions>{sheet.term_file_path && <button type="button" className="action-button" onClick={() => void downloadSignedTerm(sheet)}><Eye size={14} /> Visualizar termo</button>}<label className="action-button upload-term-button"><Upload size={14} /> {uploadingId === sheet.id ? "Enviando..." : sheet.term_file_path ? "Trocar termo assinado" : "Anexar termo assinado"}<input type="file" accept="application/pdf,image/jpeg,image/png,image/webp" onChange={(event) => void uploadTerm(sheet, event)} disabled={uploadingId === sheet.id} /></label></td></tr>)}</tbody></table></div> : <div className="empty-state"><ShieldCheck size={27} /><strong>Nenhuma ficha registrada</strong><span>As fichas aparecerão após cada entrega.</span></div>}</section>

      <section className="sheet-section employee-return-sheets"><div className="sheet-section-heading"><div><p>FICHAS DE DEVOLUÇÃO</p><h2>Termos de devolução</h2><span>{returnSheets.length} termo(s) registrado(s)</span></div></div>{returnSheets.length ? <div className="table-wrap"><table><thead><tr><th>DOCUMENTO</th><th>DATA</th><th>MATERIAIS</th><th>STATUS</th><th>AÇÕES</th></tr></thead><tbody>{returnSheets.map((sheet) => <tr key={sheet.id}><td><strong>Termo de devolução</strong><small>{reasonLabels[sheet.reason] || sheet.reason}</small></td><td>{date(sheet.returned_at)}</td><td>{sheet.items.map((item, index) => <span className="employee-term-material" key={`${sheet.id}-${index}`}>{item.quantity}x {item.material?.name || "Material"}{item.variant ? ` · Tamanho ${item.variant.size || item.variant.name}` : ""}</span>)}</td><td><span className={`status-pill ${sheet.term_signed_at ? "success" : "warning"}`}>{sheet.term_signed_at ? <><Check size={11} /> {sheet.term_signature_method === "physical_upload" ? "Assinado fisicamente" : "Assinado digitalmente"}</> : "Pendente"}</span></td><td><EmployeeTermActions><div className="employee-sheet-actions"><button type="button" className="action-button" onClick={() => void downloadReturnTerm(sheet)}><Download size={14} /> Baixar para assinatura</button>{!sheet.term_signed_at && <ReturnSignatureModal returnId={sheet.id} employeeName={employee!.full_name} employeeCpf={employee!.cpf} employeeRegistration={employee!.registration} returnedAt={sheet.returned_at} reason={sheet.reason} items={sheet.items} currentPath={sheet.term_file_path} onComplete={(path) => setReturnSheets((current) => current.map((item) => item.id === sheet.id ? { ...item, term_file_path: path, term_signature_method: "assisted", term_signed_at: new Date().toISOString() } : item))} />}</div></EmployeeTermActions>{sheet.term_file_path && <button type="button" className="action-button" onClick={() => void openSignedReturnTerm(sheet)}><Eye size={14} /> Visualizar termo</button>}<label className="action-button upload-term-button"><Upload size={14} /> {uploadingId === sheet.id ? "Enviando..." : sheet.term_file_path ? "Trocar termo assinado" : "Anexar termo assinado"}<input type="file" accept="application/pdf,image/jpeg,image/png,image/webp" onChange={(event) => void uploadReturnTerm(sheet, event)} disabled={uploadingId === sheet.id} /></label></td></tr>)}</tbody></table></div> : <div className="empty-state"><ShieldCheck size={27} /><strong>Nenhum termo de devolução</strong><span>Os termos aparecerão após uma devolução registrada.</span></div>}</section>

      <section className="sheet-section"><div className="sheet-section-heading"><div><p>RESPONSABILIDADE ATUAL</p><h2>Materiais atualmente com o colaborador</h2><span>{currentItems.length} registro(s) em posse do colaborador</span></div></div>{currentItems.length ? <div className="table-wrap sheet-table-wrap"><table><thead><tr><th>MATERIAL</th><th>IDENTIFICAÇÃO</th><th>VALIDADE</th><th>LOTE</th><th>QTD.</th><th>ENTREGA</th><th>PREVISÃO DE TROCA</th><th>STATUS</th><th>MOTIVO</th></tr></thead><tbody>{currentItems.map((item) => { const status = replacementStatus(item.expected_replacement_at); return <tr key={item.id}><td><strong>{item.material?.name || "Material"}</strong><small>{item.material?.internal_code ? `${item.material.internal_code || "Código não informado"} · ${item.material.unit}` : item.material?.unit}</small></td><td>{item.units.length ? item.units.map((unit, index) => <span className="employee-term-material" key={`${item.id}-unit-${index}`}>{unit.unit_identifier}</span>) : "—"}</td><td>{item.units.length ? item.units.map((unit, index) => <span className="employee-term-material" key={`${item.id}-validity-${index}`}>{date(unit.valid_until)}</span>) : "—"}</td><td>{item.lot?.lot_number || "—"}</td><td><strong>{item.quantity - item.returned}</strong></td><td>{date(item.delivery?.delivered_at ?? null)}</td><td>{date(item.expected_replacement_at)}</td><td><span className={`status-pill ${status.tone}`}>{status.tone !== "success" && status.tone !== "neutral" && <AlertTriangle size={11} />}{status.label}</span></td><td>{reasonLabels[item.delivery?.reason ?? ""] || "—"}</td></tr>; })}</tbody></table></div> : <div className="empty-state"><ShieldCheck size={27} /><strong>Nenhum material em posse</strong><span>Este colaborador não possui materiais atualmente sob sua responsabilidade.</span></div>}</section>
