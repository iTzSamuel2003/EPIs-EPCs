  ...
  useEffect(() => {
    async function load() {
      ...
      setItems(((deliveryData ?? []) as unknown[]).map((row) => {
        ...
        return { id: String(value.id), quantity: Number(value.quantity), expected_replacement_at: value.expected_replacement_at as string | null, material, lot: value.material_lots as Item["lot"], units, delivery: value.deliveries as Item["delivery"] };
      }));
      setLoading(false);
    }
    void load();
