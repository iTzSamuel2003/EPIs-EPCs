...
function EmployeeTermActions({ children }: { children: ReactNode }) {
  ...
  return <div className="employee-term-menu"><button type="button" className="action-button employee-term-menu-trigger" onClick={() => setOpen((current) => !current)} aria-expanded={open}>Ações do termo</button>{open && <div className="employee-term-menu-panel">{children}</div>}</div>;
}
