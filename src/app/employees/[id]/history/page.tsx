...
export default function EmployeeHistoryPage() {
  ...
  const filteredEvents = useMemo(() => events.filter((event) => { const eventDate = dateKey(event.date); return (kind === "all" || event.kind === kind) && (!materialFilter || event.material === materialFilter) && (!from || (eventDate && eventDate >= from)) && (!to || (eventDate && eventDate <= to)); }), [events, from, kind, materialFilter, to]);
