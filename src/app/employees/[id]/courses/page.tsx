...
import { ArrowLeft, BookOpenCheck, Check, CheckCircle2, ClipboardList, LoaderCircle, Plus, Trash2, X } from "lucide-react";
...
function hasValidCourse(courses: Course[], requirement: Requirement) { return courses.some((course) => normalize(course.name).includes(normalize(requirement.course_name.split(" - ")[0])) && (!course.expires_at || course.expires_at >= new Date().toISOString().slice(0, 10))); }
...
export default function EmployeeCoursesPage() {
  ...
  const requiredCourses = useMemo(() => requirements.filter((requirement) => matchesGroup(functionName, requirement.function_group)), [requirements, functionName]);
  const pendingRequired = requiredCourses.filter((requirement) => !hasValidCourse(courses, requirement)).length;
