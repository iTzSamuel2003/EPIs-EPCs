...
import { Check, FileText, GraduationCap, LoaderCircle, Pencil, Plus, Ruler, Search, UserRound, X } from "lucide-react";
import { useRouter } from "next/navigation";
import { createClient } from "@/lib/supabase/client";

type Employee = { id: string; registration: string | null; full_name: string; cpf: string; job_title: string | null; function_name: string | null; department: string | null; unit: string | null; admission_date: string | null; phone: string | null; email: string | null; status: "active" | "away" | "terminated"; notes: string | null };
type EmployeeForm = { registration: string; full_name: string; cpf: string; cargo_funcao: string; department: string; unit: string; admission_date: string; phone: string; email: string; status: "active" | "away" | "terminated"; notes: string };

const emptyForm: EmployeeForm = { registration: "", full_name: "", cpf: "", cargo_funcao: "", department: "", unit: "", admission_date: "", phone: "", email: "", status: "active", notes: "" };
...
export default function EmployeesPage() {
  ...
  async function loadEmployees() {
    setLoading(true);
    const { data, error: loadError } = await createClient().from("employees").select("id,registration,full_name,cpf,job_title,function_name,department,unit,admission_date,phone,email,status,notes").order("full_name");
  ...
  const filtered = employees.filter((employee) => `${employee.full_name} ${employee.registration ?? ""} ${employee.cpf} ${employee.department ?? ""} ${employee.function_name ?? ""}`.toLowerCase().includes(query.toLowerCase()) && (statusFilter === "all" || employee.status === statusFilter));
  ...
  async function saveEmployee(event: FormEvent) {
    ...
    const { error: insertError } = await supabase.from("employees").insert({
      ...
      job_title: null, function_name: form.cargo_funcao.trim() || null, department: form.department.trim() || null, unit: form.unit || null,
  ...
  return <main className="module-shell">
    ...
    <section className="panel module-table-card employees-table-card"><div className="panel-header"><div><h2>Colaboradores</h2><p>{filtered.length} resultado(s) encontrados</p></div></div>{loading ? <div className="module-loading"><LoaderCircle className="spin" size={22} /> Carregando funcionários...</div> : <div className="table-wrap"><table><thead><tr><th>FUNCIONÁRIO</th><th>MATRÍCULA</th><th>CARGO / FUNÇÃO</th><th>UNIDADE</th><th>STATUS</th><th>AÇÕES</th></tr></thead><tbody>{filtered.map((employee) => { const [label, tone] = statusLabels[employee.status]; return <tr key={employee.id}><td><div className="material-cell"><div className="employee-avatar"><UserRound size={16} /></div><div><strong>{employee.full_name}</strong><small>CPF {employee.cpf}</small></div></div></td><td>{employee.registration || <span className="muted-cell">Pendente</span>}</td><td>{employee.job_title || employee.function_name || <span className="muted-cell">Não informado</span>}</td><td>{employee.unit || <span className="muted-cell">Não informada</span>}</td><td><span className={`status-pill ${tone}`}>{label}</span></td><td><div className="row-actions"><button className="action-button" onClick={() => router.push(`/employees/${employee.id}`)}><FileText size={14} /> Ficha</button><button className="action-button" onClick={() => router.push(`/employees/${employee.id}/edit`)}><Pencil size={14} /> Editar</button><button className="action-button" onClick={() => router.push(`/employees/${employee.id}/profile`)}><Ruler size={14} /> Medidas</button><button className="action-button" onClick={() => router.push(`/employees/${employee.id}/profile`)}><GraduationCap size={14} /> Cursos</button></div></td></tr>; })}</tbody></table>{!filtered.length && <div className="empty-state"><UserRound size={27} /><strong>Nenhum funcionário encontrado</strong><span>Ajuste os filtros ou cadastre o primeiro colaborador.</span></div>}</div>}</section>
