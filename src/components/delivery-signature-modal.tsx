...
function cropSignature(canvas: HTMLCanvasElement) { const context = canvas.getContext("2d"); if (!context) return { dataUrl: canvas.toDataURL("image/png"), width: canvas.width, height: canvas.height }; const pixels = context.getImageData(0, 0, canvas.width, canvas.height).data; let left = canvas.width; let top = canvas.height; let right = -1; let bottom = -1; for (let y = 0; y < canvas.height; y += 1) for (let x = 0; x < canvas.width; x += 1) if (pixels[(y * canvas.width + x) * 4 + 3] > 12) { left = Math.min(left, x); top = Math.min(top, y); right = Math.max(right, x); bottom = Math.max(bottom, y); } if (right < 0) return { dataUrl: canvas.toDataURL("image/png"), width: canvas.width, height: canvas.height }; const padding = 12; const sourceX = Math.max(0, left - padding); const sourceY = Math.max(0, top - padding); const sourceWidth = Math.min(canvas.width - sourceX, right - left + 1 + padding * 2); const sourceHeight = Math.min(canvas.height - sourceY, bottom - top + 1 + padding * 2); const cropped = document.createElement("canvas"); cropped.width = sourceWidth; cropped.height = sourceHeight; cropped.getContext("2d")?.drawImage(canvas, sourceX, sourceY, sourceWidth, sourceHeight, 0, 0, sourceWidth, sourceHeight); return { dataUrl: cropped.toDataURL("image/png"), width: sourceWidth, height: sourceHeight }; }
...
export function DeliverySignatureModal({ deliveryId, employeeName, employeeCpf, employeeRegistration, deliveredAt, reason, items, currentPath, onComplete }: DeliverySignatureModalProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null); const modalRef = useRef<HTMLElement>(null);
  ...
  async function submit() {
    ...
    if (!auth.user) { await supabase.storage.from("delivery-terms").remove([path]); setError("Sua sessão expirou. Entre novamente."); return; } const { data: updatedDelivery, error: updateError } = await supabase.from("deliveries").update({ term_file_path: path, term_uploaded_at: new Date().toISOString(), term_uploaded_by: auth.user.id, term_signature_method: "assisted", term_signed_at: new Date().toISOString(), term_signer_name: employeeName, term_signer_cpf: digits(cpf) }).eq("id", deliveryId).select("id").maybeSingle();
    if (updateError || !updatedDelivery) { setError(friendlyError(updateError, "Não foi possível salvar a assinatura. Verifique se você ainda tem permissão para esta ficha.")); return; }
  ...
  return <><div className="delivery-attachment-actions"><button type="button" className="action-button assisted-signature-button" onClick={() => setOpen(true)}><PenLine size={14} /> Assinar no aparelho</button></div>{open && <div className="signature-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}><section ref={modalRef} className="signature-modal" tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="signature-title" aria-describedby="signature-help"><button className="signature-modal-close" type="button" onClick={close} aria-label="Fechar janela de assinatura"><X size={18} aria-hidden="true" /></button><div className="signature-modal-icon" aria-hidden="true"><PenLine size={22} /></div><h2 id="signature-title">Assinatura assistida</h2><p id="signature-help">O colaborador deve conferir os materiais, informar o CPF e assinar no campo abaixo.</p><label htmlFor="delivery-signature-cpf">CPF do colaborador<input id="delivery-signature-cpf" value={cpf} onChange={(event) => setCpf(event.target.value)} inputMode="numeric" placeholder="000.000.000-00" aria-invalid={Boolean(error)} aria-describedby={error ? "delivery-signature-error" : "signature-help"} /></label><div className="signature-pad-label"><span id="delivery-signature-pad-label">Assinatura</span><button type="button" onClick={clearSignature}><Eraser size={13} aria-hidden="true" /> Limpar</button></div><canvas ref={canvasRef} width={640} height={180} className="signature-pad" aria-label="Campo para desenhar a assinatura" aria-describedby="delivery-signature-pad-label" tabIndex={0} onPointerDown={startDrawing} onPointerMove={draw} onPointerUp={() => setDrawing(false)} onPointerCancel={() => setDrawing(false)} /><label className="signature-acceptance" htmlFor="delivery-signature-acceptance"><input id="delivery-signature-acceptance" type="checkbox" checked={accepted} onChange={(event) => setAccepted(event.target.checked)} /> Confirmo que o colaborador leu e concorda com o termo de entrega e responsabilidade.</label>{error && <div id="delivery-signature-error" className="feedback error-feedback" role="alert" aria-live="assertive"><X size={15} aria-hidden="true" /> <span className="feedback-copy">{error}</span></div>}<div className="signature-modal-actions"><button type="button" className="secondary-button" onClick={close}>Cancelar</button><button type="button" className="primary-button" onClick={() => void submit()} disabled={saving} aria-busy={saving}>{saving ? "Gerando documento..." : "Confirmar assinatura"}<Check size={16} aria-hidden="true" /></button></div></section></div>}</>;
}
