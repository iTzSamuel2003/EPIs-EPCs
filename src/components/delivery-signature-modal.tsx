...
export function DeliverySignatureModal({ deliveryId, employeeName, employeeCpf, employeeRegistration, deliveredAt, reason, items, currentPath, onComplete }: DeliverySignatureModalProps) {
  ...
  async function submit() {
    ...
    if (uploadError) { setError(friendlyError(uploadError, "Não foi possível salvar o termo.")); setSaving(false); return; }
    ...
    if (updateError) { await supabase.storage.from("delivery-terms").remove([path]); setError(friendlyError(updateError, "Não foi possível salvar a assinatura.")); setSaving(false); return; }
    const { error: previousAttachmentError } = currentPath ? await supabase.storage.from("delivery-terms").remove([currentPath]) : { error: null };
    onComplete(path); close(); setSaving(false); if (previousAttachmentError) setError("Assinatura salva, mas o termo anterior não pôde ser removido.");
  }

  return <><div className="delivery-attachment-actions"><button type="button" className="action-button assisted-signature-button" onClick={() => setOpen(true)}><PenLine size={14} /> Assinar no aparelho</button></div>{open && <div className="signature-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}><section ref={modalRef} className="signature-modal" tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="signature-title"><button className="signature-modal-close" type="button" onClick={close} aria-label="Fechar"><X size={18} /></button><div className="signature-modal-icon"><PenLine size={22} /></div><h2 id="signature-title">Assinatura assistida</h2><p>O colaborador deve conferir os materiais, informar o CPF e assinar no campo abaixo.</p><label>CPF do colaborador<input value={cpf} onChange={(event) => setCpf(event.target.value)} inputMode="numeric" placeholder="000.000.000-00" /></label><div className="signature-pad-label"><span>Assinatura</span><button type="button" onClick={clearSignature}><Eraser size={13} /> Limpar</button></div><canvas ref={canvasRef} width={640} height={180} className="signature-pad" aria-label="Campo para desenhar a assinatura" tabIndex={0} onPointerDown={startDrawing} onPointerMove={draw} onPointerUp={() => setDrawing(false)} onPointerCancel={() => setDrawing(false)} /><label className="signature-acceptance"><input type="checkbox" checked={accepted} onChange={(event) => setAccepted(event.target.checked)} /> Confirmo que o colaborador leu e concorda com o termo de entrega e responsabilidade.</label>{error && <div className="feedback error-feedback" role="alert"><X size={15} /> {error}</div>}<div className="signature-modal-actions"><button type="button" className="secondary-button" onClick={close}>Cancelar</button><button type="button" className="primary-button" onClick={() => void submit()} disabled={saving}>{saving ? "Gerando documento..." : "Confirmar assinatura"}<Check size={16} /></button></div></section></div>}</>;
}
