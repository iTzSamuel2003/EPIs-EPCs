...
export function RecentDeliveries() {
  ...
  useEffect(() => {
    async function load() {
      const { data, error: loadError } = await createClient().from("deliveries").select("id,delivered_at,reason,notes,employee:employees(full_name,registration),delivery_items(quantity,expected_replacement_at,material:materials(name,unit,internal_code))").order("delivered_at", { ascending: false }).order("created_at", { ascending: false }).limit(10);
