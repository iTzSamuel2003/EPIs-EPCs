...
begin
  ...
  insert into public.function_templates (organization_id, name, source_document) values (v_org, 'Eletricista de linha morta III', 'Baseado na ficha de Eletricista de linha morta I') returning id into v_template;
  insert into public.function_template_items (organization_id, template_id, material_name, quantity, item_type)
  select v_org, v_template, material_name, quantity, item_type from public.function_template_items where template_id = (select id from public.function_templates where organization_id=v_org and name='Eletricista de linha morta I');
  insert into public.function_template_items (organization_id, template_id, material_name, quantity, item_type) values (v_org, v_template, 'Chave Phillips', 1, 'FERRAMENTAL'), (v_org, v_template, 'Chave de fenda', 1, 'FERRAMENTAL'), (v_org, v_template, 'Canivete sem ponta', 1, 'FERRAMENTAL');
end $$;
