-- Do not reveal employee data after a one-time measurement link is consumed.
...
-- Serialize submissions for the same token so a one-time link cannot be used twice concurrently.
