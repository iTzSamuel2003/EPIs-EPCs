-- Guarda as datas do ensaio no item entregue ao colaborador.
alter table public.delivery_items add column if not exists test_performed_at date;
alter table public.delivery_items add column if not exists test_expires_at date;
alter table public.delivery_items add constraint delivery_items_test_dates_check
...
create or replace function public.register_delivery(
  p_employee_id uuid, p_reason text, p_delivered_at date,
...
declare
  ...
  v_valid_until date; v_test_performed_at date; v_test_expires_at date;
  v_lot record; v_material_row record; unit_number int;
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    v_material := (v_item->>'material_id')::uuid; v_variant := nullif(v_item->>'variant_id', '')::uuid; v_requested := (v_item->>'quantity')::int; v_test_performed_at := nullif(v_item->>'test_performed_at', '')::date; v_test_expires_at := nullif(v_item->>'test_expires_at', '')::date;
  ...
  insert into public.deliveries (organization_id, employee_id, delivered_at, reason, responsible_id, notes) values (v_org, p_employee_id, coalesce(p_delivered_at, current_date), p_reason, auth.uid(), nullif(trim(p_notes), '')) returning id into v_delivery;
  for v_item in select * from jsonb_array_elements(p_items) loop
    v_material := (v_item->>'material_id')::uuid; v_variant := nullif(v_item->>'variant_id', '')::uuid; v_requested := (v_item->>'quantity')::int; v_remaining := v_requested; v_test_performed_at := nullif(v_item->>'test_performed_at', '')::date; v_test_expires_at := nullif(v_item->>'test_expires_at', '')::date;
    ...
    for v_lot in select id, lot_number, available_quantity from public.material_lots where organization_id = v_org and material_id = v_material and (variant_id = v_variant or (variant_id is null and v_variant is null)) and available_quantity > 0 and (expires_at is null or expires_at >= current_date) order by expires_at asc nulls last, entry_date asc, created_at asc for update loop
      exit when v_remaining = 0; v_take := least(v_remaining, v_lot.available_quantity); update public.material_lots set available_quantity = available_quantity - v_take where id = v_lot.id;
      insert into public.delivery_items (organization_id, delivery_id, material_id, variant_id, lot_id, quantity, expected_replacement_at, test_performed_at, test_expires_at) values (v_org, v_delivery, v_material, v_variant, v_lot.id, v_take, nullif(v_item->>'expected_replacement_at', '')::date, v_test_performed_at, v_test_expires_at) returning id into v_delivery_item;
      if v_material_row.test_required then for unit_number in 1..v_take loop insert into public.material_units (organization_id, material_id, unit_identifier, lot_number, size, status, location, notes, delivery_item_id, employee_id, delivered_at, valid_until) values (v_org, v_material, 'MU-' || lpad(nextval('public.material_unit_identifier_seq')::text, 8, '0'), v_lot.lot_number, null, 'assigned', null, 'Gerado automaticamente na entrega ' || v_delivery::text, v_delivery_item, p_employee_id, coalesce(p_delivered_at, current_date), coalesce(v_test_expires_at, v_valid_until)); end loop; end if;
      insert into public.stock_movements (organization_id, material_id, lot_id, movement_type, quantity, created_by, notes) values (v_org, v_material, v_lot.id, 'delivery', v_take, auth.uid(), 'Entrega ' || v_delivery::text); v_remaining := v_remaining - v_take;
    end loop;
